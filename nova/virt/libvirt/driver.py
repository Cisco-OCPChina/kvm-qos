    ...
    def partition(self, hypervisor, component, instance_list, percentage):
        if cmp(component, 'cpu') == 0:
            self._do_cpu_partition(hypervisor, instance_list, percentage)

    def _do_cpu_partition(self, hypervisor, instance_list, percentage):
        dirs = '/cgroup/cpu/libvirt/qemu/'
        for i in instance_list:
            quotas = int(100000 * float(i['percentage']) )
